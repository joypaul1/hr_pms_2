...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && trim($_POST["actionType"]) == 'searchUser') {
    ...
    $strSQL = oci_parse(
        $objConnect,
        "SELECT ID,EMP_NAME,MOBILE_NO,RML_ID,R_CONCERN,DEPT_NAME,DESIGNATION,DEPT_HEAD_RML_ID  FROM RML_HR_APPS_USER WHERE RML_ID LIKE '%" . trim($_POST['search']) . "%' FETCH FIRST 10 ROWS ONLY"
    );
    @oci_execute($strSQL);
