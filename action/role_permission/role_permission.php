...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&  trim($_POST["actionType"]) == 'create') {
    ...
    if (empty($name)) {
        ...
        header("location:" . $basePath . "/role_permission/role_permissions/create.php");
        exit;
    } else {
        $sql = "INSERT INTO  tbl_roles_permissions ( name, slug) VALUES (?, ?)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&  trim($_POST["actionType"]) == 'update') {
    ...
    if (empty($name)) {
        ...
        $_SESSION['noti_message'] = $message;
        header("location:" . $basePath . "/role_permission/role_permissions/edit.php?id=" . $id . "&amp;&amp;actionType=edit");
        exit;
    } else {
        $sql = "UPDATE tbl_roles_permissions SET name=?, slug=? WHERE id=?";
