...
<body>
    <div class="stamp-paper">
        ...
        <section class="stamp-body" style="padding: 0 5px 0 5px;">
           ...
           <span> <b><?php echo $_POST['customer_name'] ?>, S/O: <?php echo $_POST['c_f_name'] ?>, Add: <?php  echo $_POST['customer_address'] ?>.</b> Hereinafter referred to as "the BORROWER", (which expression shall where the context so admits mean and include heirs, legal representatives, executors, successors and assigns) of the <u style="font-weight: 600;">SECOND PART</u>".</span>
           <p style="text-align: center;font-weight: 600;margin: 3px 0 3px 0px">AND</p>
           ...
           <span style="display: block; margin-top: 5px;">WHEREAS the First Part (the owner) is an importer and owners of <b><?php echo str_pad($_POST['unit_no'], 2, '0', STR_PAD_LEFT) ?> (<?php echo ucwords($numberTransformer->toWords($_POST['unit_no'])) ?>)</b> unit/units 
           Completely Built up <b><?php echo $_POST['product_model'] ?></b> With fitting, tools and accessories as fully described in Schedule "A" attached hereto for marketing and selling the same the same in Bangladesh.</span> 

            <span style="display: block; margin-top: 5px;">AND WHEREAS, the Second Part wishes to procure on Hire-Purchase form the First part(the Owners) 01(One) Unit/Units Completely Built up and has applied to the first part and the First Part (the Owners) is willing to sell on hire-purchase to the Second Part (The Borrower) the aforesaid chasis as described in the "A" Schedule set forth hereto.</span>
            ...
            <b><?php echo $_POST['product_model'] ?></b> with fitting tootls and accessories for a sum of <b><?php echo number_format($_POST['sales_amount'], 2) ?> (<?php echo ucwords($numberTransformer->toWords($_POST['sales_amount'])) ?>) </b> only each as fully described in schedule "A" attached hereto plus additional charges due to hire purchase deal given in <b>CLAUSE VII.</b>
            </span>
            <p style="text-align: left;font-weight: 600;margin: 3px 0 3px 0px">CLAUSE II</p>
            <span style="display: block; margin-top: 5px;">That the Second Part shall pay to the First Part (the Owners) at the time of execution and signing of this Agreement a sum of <b>TK. <?php echo number_format($_POST['down_payment'], 2) ?> (<?php echo ucwords($numberTransformer->toWords($_POST['down_payment'])) ?>)</b> only <b> Down Payment</b> and thereafter the Second Part will punctually and duly pay to the First Part (Owners) at their address in cash /cheque/pay order/demand draft/telephonic transfer the sums <b><u>and on the dates</u></b> mentioned in Schedule "B" <b><u>annexed</u></b> hereto, whether previously demanded or not by way of installment payment for the product.</span>
